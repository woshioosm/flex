package org.liesmars.flashwebclient.Handler.Edit
{
	...
	public class EnlargeVector extends Edit
	{
		public function EnlargeVector(map:GeoMap, options:Object)
		{
			super(map, options);
		}
...
//			}
					...
					for(var j:int=0;j<slayer.sprite.numChildren;j++)
					{
						...
						if(shape) {
							...
							if(shape.geotype != GEOTYPES.POINT){
								shape.addEventListener(MouseEvent.CLICK,Enlarge);
							}				
		...
		public function Enlarge(e:MouseEvent):void{
			trace("enlarge");
			...
			for(var i:int = 0;i<arr2.length;i=i+2){
				arr2[i] = ComputeCoordinate(arr2[i],averagex);
				arr2[i+1] = ComputeCoordinate(arr2[i+1],averagey);
				trace("!!");
		...
		public function ComputeCoordinate(target:Number,center:Number):Number{
			return 1.3*target-0.3*center;
		}
